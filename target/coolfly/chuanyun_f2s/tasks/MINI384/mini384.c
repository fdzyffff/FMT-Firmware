...
{
    // DLOG_Critical("mini384_loop_callback %i %i",CPUINFO_GetLocalCpuId(),&is_Inited);
...
int mini384_encode_config(void)
{
    ...
    sys_msleep(1000); //延时一段时间让CPU2操作完DVP
...
static void mini384_init(void)
{
    ...
    uint8_t config1_640x512_30fps[10] = { 0x9D, 0x00, 0x0F, 0xc1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08 };
    uint8_t config2_640x512_30fps[10] = { 0x1D, 0x08, 0x00, 0x00, 0x02, 0x80, 0x02, 0x00, 0x1E, 0x00 };
    HAL_I2C_MasterWriteData(MINI384_I2C, MINI384_I2C_ADDR, config1_640x512_30fps, 10, 1000);
    HAL_Delay(20);
    HAL_I2C_MasterWriteData(MINI384_I2C, MINI384_I2C_ADDR, config2_640x512_30fps, 10, 1000);
    HAL_Delay(20);
    ...
    // HAL_Delay(20);
    HAL_I2C_MasterInit(MINI384_I2C, MINI384_I2C_ADDR, HAL_I2C_STANDARD_SPEED);
