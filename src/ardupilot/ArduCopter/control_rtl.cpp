...
void Copter::rtl_climb_start()
{
    ...
    console_printf(" rtl_climb_start()\n");
}
...
void Copter::rtl_loiterathome_run()
{
    ...
    if ((millis() - rtl_loiter_start_time) >= (uint32_t)g.rtl_loiter_time) {
        if (auto_yaw_mode == AUTO_YAW_RESETTOARMEDYAW) {
            ...
            float delta_angle_float = (float)(ahrs.yaw_sensor-initial_armed_bearing)*0.1f;
            if (fabsf(wrap_180(delta_angle_float)) <= 2.0f) {
...
void Copter::rtl_build_path(bool terrain_following_allowed)
{
    ...
    Vector3f rtl_origin_v3f;
    if (rtl_path.origin_point.get_vector_from_origin_NEU(rtl_origin_v3f)) {
        console_printf(" rtl_origin_v3f[%f,%f,%f]\n", rtl_origin_v3f.x, rtl_origin_v3f.y, rtl_origin_v3f.z);
    }
    Vector3f rtl_climb_v3f;
    if (rtl_path.climb_target.get_vector_from_origin_NEU(rtl_climb_v3f)) {
        console_printf(" rtl_climb_v3f[%f,%f,%f]\n", rtl_climb_v3f.x, rtl_climb_v3f.y, rtl_climb_v3f.z);
    }
    Vector3f rtl_decent_v3f;
    if (rtl_path.descent_target.get_vector_from_origin_NEU(rtl_decent_v3f)) {
        console_printf(" rtl_decent_v3f[%f,%f,%f]\n", rtl_decent_v3f.x, rtl_decent_v3f.y, rtl_decent_v3f.z);
    }
    // set land flag
...
void Copter::rtl_compute_return_target(bool terrain_following_allowed)
{
    ...
    rtl_path.terrain_used = false;//terrain_use() && terrain_following_allowed;
    if (rtl_path.terrain_used) {
