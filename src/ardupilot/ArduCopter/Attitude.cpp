...
void Copter::get_pilot_desired_lean_angles(float roll_in, float pitch_in, float &roll_out, float &pitch_out, float angle_max)
{
    ...
    aparm.angle_max = apm_constrain_int16(aparm.angle_max,1000,8000);

    // limit max lean angle
    angle_max = apm_constrain_float(angle_max, 1000, aparm.angle_max);
...
void Copter::update_throttle_hover()
{
    ...
    if (mode_has_manual_throttle(control_mode) || (control_mode == control_mode_t::DRIFT)) {
        return;
    }
    ...
    if (throttle > 0.0f && abs(climb_rate) < 60 && labs(ahrs.roll_sensor) < 500 && labs(ahrs.pitch_sensor) < 500) {
        ...
        motors->update_throttle_hover(0.0025f);
    }
}
...
float Copter::get_pilot_desired_throttle(int16_t throttle_control, float thr_mid)
{
    ...
    throttle_control = apm_constrain_int16(throttle_control,0,1000);
    ...
    float expo = apm_constrain_float(-(thr_mid-0.5)/0.375, -0.5f, 1.0f);
...
float Copter::get_pilot_desired_climb_rate(float throttle_control)
{
    ...
    throttle_control = apm_constrain_float(throttle_control,0.0f,1000.0f);

    // ensure a reasonable deadzone
    g.throttle_deadzone = apm_constrain_int16(g.throttle_deadzone, 0, 400);
...
void Copter::set_accel_throttle_I_from_pilot_throttle()
{
    ...
    float pilot_throttle = apm_constrain_float(attitude_control->get_throttle_in(), 0.0f, 1.0f);
