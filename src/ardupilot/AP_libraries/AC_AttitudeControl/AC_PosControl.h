...
#define POSCONTROL_ACCEL_XY                     150.0f  // default horizontal acceleration in cm/s/s.  This is overwritten by waypoint and loiter controllers
...
#define POSCONTROL_SPEED                        1000.0f  // default horizontal speed in cm/s
...
#define POSCONTROL_VEL_XY_MAX_FROM_POS_ERR      400.0f  // max speed output from pos_to_vel controller when feed forward is used
...
public:
    ...
    float get_accel_z() const { return _accel_z_cms; }
		...
		float get_posecontrol_offset() const {return _posecontrol_offset; }
    ...
    void init_xy_controller_zero(bool reset_I = true);
    ...
    /// set_z_target in cm from home
    void set_z_target(float z);
    ...
    float calc_leash_length(float speed_cms) const;

    void set_further_limit(float v) {_further_limit = v;}

    // parameters
    float    _accel_xy_filt_hz;     //param // XY acceleration filter cutoff frequency
