...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    _accel_xy_filt_hz(POSCONTROL_ACCEL_FILTER_HZ),
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    _p_pos_z(p_pos_z),
    _p_vel_z(p_vel_z),
...
void AC_PosControl::set_alt_target_with_slew(float alt_cm, float dt)
{
    ...
    if ((alt_change<0 && !_motors.limit.throttle_lower) || (alt_change>0 && !_motors.limit.throttle_upper)) {
        if (!is_zero(dt)) {
            float climb_rate_cms = apm_constrain_float(alt_change/dt, _speed_down_cms, _speed_up_cms);
    ...
    _pos_target.z = apm_constrain_float(_pos_target.z,curr_alt-_leash_down_z,curr_alt+_leash_up_z);
}
...
void AC_PosControl::set_alt_target_from_climb_rate_ff(float climb_rate_cms, float dt, bool force_descend)
{
    ...
    accel_z_cms = apm_constrain_float(accel_z_cms, 0.0f, 750.0f);
    ...
    _vel_desired.z = apm_constrain_float(climb_rate_cms, _vel_desired.z-vel_change_limit, _vel_desired.z+vel_change_limit);
    _flags.use_desvel_ff_z = true;
...
void AC_PosControl::get_stopping_point_z(Vector3f& stopping_point) const
{
    ...
    stopping_point.z = apm_constrain_float(stopping_point.z, curr_pos_z - POSCONTROL_STOPPING_DIST_DOWN_MAX, curr_pos_z + POSCONTROL_STOPPING_DIST_UP_MAX);
}
...
void AC_PosControl::accel_to_throttle(float accel_target_z)
{
    ...
    if ((!_motors.limit.throttle_lower && !_motors.limit.throttle_upper) || (i>0&&_accel_error.z<0) || (i<0&&_accel_error.z>0)) {
        if ( ((p+i+d)/1000.0f +_motors.get_throttle_hover()) > _further_limit ) {
    ...
    thr_out = MAX(thr_out, _further_limit);
...
void AC_PosControl::get_stopping_point_xy(Vector3f &stopping_point) const
{
    ...
    stopping_dist = apm_constrain_float(stopping_dist, 0, _leash);
...
void AC_PosControl::pos_to_rate_xy(xy_mode mode, float dt, float ekfNavVelGainScaler)
{
    ...
    if (kP <= 0.0f) {
        ...
        linear_distance = _accel_cms/(2.0f*kP*kP);             //5.5m

        if (_distance_to_target > 2.0f*linear_distance) {               //11m
            ...
            _vel_target.x = kP * _pos_error.x;               //3
...
void AC_PosControl::accel_to_lean_angles(float dt, float ekfNavVelGainScaler, bool use_althold_lean_angle)
{
    ...
    if (use_althold_lean_angle) {
        accel_max = MIN(accel_max, GRAVITY_MSS * 100.0f * tanf(ToRad(apm_constrain_float(_attitude_control.get_althold_lean_angle_max(),1000,8000)/100.0f)));
    }
    ...
    _pitch_target = apm_constrain_float(atanf(-accel_forward/(GRAVITY_MSS * 100))*(18000/M_PI),-lean_angle_max, lean_angle_max);
    float cos_pitch_target = cosf(_pitch_target*M_PI/18000);
    _roll_target = apm_constrain_float(atanf(accel_right*cos_pitch_target/(GRAVITY_MSS * 100))*(18000/M_PI), -lean_angle_max, lean_angle_max);
}
