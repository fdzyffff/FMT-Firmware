...
void AP_MotorsMatrix::output_armed_stabilizing()
{
    ...
    _throttle_avg_max = apm_constrain_float(_throttle_avg_max, throttle_thrust, _throttle_thrust_max);
    ...
    if (fabsf(yaw_thrust) > yaw_allowed) {
        yaw_thrust = apm_constrain_float(yaw_thrust, -yaw_allowed, yaw_allowed);
        limit.yaw = true;
    }
    ...
    for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            _thrust_rpyt_out[i] = apm_constrain_float(_thrust_rpyt_out[i], 0.0f, 1.0f);
        }
    }
}
