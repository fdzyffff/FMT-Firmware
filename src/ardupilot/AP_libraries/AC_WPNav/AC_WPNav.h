...
public:
    ...
    void update_loiter_stopping(float ekfGndSpdLimit, float ekfNavVelGainScaler);
    ...
    float get_wp_acceleration() const { return _wp_accel_cms; }
    ...
    // parameters
    float    _loiter_speed_cms;      //para // maximum horizontal speed in cm/s while in loiter
    float    _loiter_jerk_max_cmsss; //para // maximum jerk in cm/s/s/s while in loiter
    float    _loiter_accel_cmss;     //para // loiter's max acceleration in cm/s/s
    float    _loiter_accel_min_cmss; //para // loiter's min acceleration in cm/s/s
    float    _wp_speed_cms;          //para // maximum horizontal speed in cm/s during missions
    float    _wp_speed_up_cms;       //para // climb speed target in cm/s
    float    _wp_speed_down_cms;     //para // descent speed target in cm/s
    float    _wp_radius_cm;          //para // distance from a waypoint in cm that, when crossed, indicates the wp has been reached
    float    _wp_accel_cms;          //para // horizontal acceleration in cm/s/s during missions
    float    _wp_accel_z_cms;        //para // vertical acceleration in cm/s/s during missions
    int8_t   _rangefinder_use;       //para
