...
AC_WPNav::AC_WPNav(const AP_InertialNav& inav, const AP_AHRS_View& ahrs, AC_PosControl& pos_control, const AC_AttitudeControl& attitude_control) :
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    _loiter_speed_cms(WPNAV_LOITER_SPEED),
    _loiter_jerk_max_cmsss(WPNAV_LOITER_JERK_MAX_DEFAULT),
    _loiter_accel_cmss(WPNAV_LOITER_ACCEL),
    _loiter_accel_min_cmss(WPNAV_LOITER_ACCEL_MIN),
    _wp_speed_cms(WPNAV_WP_SPEED),
    _wp_speed_up_cms(WPNAV_WP_SPEED_UP),
    _wp_speed_down_cms(WPNAV_WP_SPEED_DOWN),
    _wp_radius_cm(WPNAV_WP_RADIUS),
    _wp_accel_cms(WPNAV_ACCELERATION),
    _wp_accel_z_cms(WPNAV_WP_ACCEL_Z_DEFAULT),
    _rangefinder_use(0),
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    _inav(inav),
    _ahrs(ahrs),
...
void AC_WPNav::calc_loiter_desired_velocity(float nav_dt, float ekfGndSpdLimit)
{
    ...
    if (!is_zero(desired_speed)) {
        ...
        if (_pilot_accel_fwd_cms == 0 && _pilot_accel_rgt_cms == 0) {
            float brake_accel = AC_AttitudeControl::sqrt_controller(desired_speed, _pos_control.get_pos_xy_kP(), _loiter_accel_cmss);
            drag_speed_delta = MIN(drag_speed_delta,-brake_accel*nav_dt);
...
bool AC_WPNav::set_wp_destination(const Vector3f& destination, bool terrain_alt)
{
    ...
    if ((millis() - _wp_last_update) < 1000) {
...
bool AC_WPNav::set_wp_origin_and_destination(const Vector3f& origin, const Vector3f& destination, bool terrain_alt)
{
    ...
    _limited_speed_xy_cms = apm_constrain_float(speed_along_track,0,_wp_speed_cms);
...
bool AC_WPNav::advance_wp_target_along_track(float dt)
{
    ...
    if (speed_along_track < -linear_velocity) {
        ...
        _limited_speed_xy_cms = apm_constrain_float(_limited_speed_xy_cms, 0.0f, _track_speed);
        ...
        if (fabsf(speed_along_track) < linear_velocity) {
            _limited_speed_xy_cms = apm_constrain_float(_limited_speed_xy_cms,speed_along_track-linear_velocity,speed_along_track+linear_velocity);
        }
    }
    ...
    if (!_flags.fast_waypoint) {
        _track_desired = apm_constrain_float(_track_desired, 0, _track_length);
    } else {
        _track_desired = apm_constrain_float(_track_desired, 0, _track_length + WPNAV_WP_FAST_OVERSHOOT_MAX);
    }
...
bool AC_WPNav::set_spline_origin_and_destination(const Vector3f& origin, const Vector3f& destination, bool terrain_alt, bool stopped_at_start, spline_segment_end_type seg_end_type, const Vector3f& next_destination)
{
    ...
    bool prev_segment_exists = (_flags.reached_destination && ((millis() - _wp_last_update) < 1000));
...
bool AC_WPNav::advance_spline_target_along_track(float dt)
{
    if (!_flags.reached_destination) {
        ...
        _spline_vel_scaler = apm_constrain_float(_spline_vel_scaler, 0.0f, vel_limit);
