...
const Vector3f AP_AHRS_NavEKF::get_gyro_latest(void) const
{
    ...
    return Vector3f(radians(fdm.rollRate),
...
void AP_AHRS_NavEKF::update_SITL(void)
{
    const struct sitl_fdm &fdm = hal.sitl_state;
        ...
        if (_home.lat == 0 && _home.lng == 0 && fdm.position_ok) {
            _home.lat = fdm.latitude;
            _home.lng = fdm.longitude;
            _home.alt = fdm.altitude*100;
            _home.options = 0;
        }
}
...
bool AP_AHRS_NavEKF::get_position(struct Location &loc) const
{
    ...
    if (fdm.position_ok) {
        loc.lat = fdm.latitude;
        loc.lng = fdm.longitude;
