...
#define EVENT_TF_LUNA_UPDATE (1 << 0)
...
{
    return rt_event_send(&event, EVENT_TF_LUNA_UPDATE);
}

static bool parse_package(uint8_t c)
{
    static TF_LUNA_State state = TF_LUNA_HEADER1;
    ...
    case TF_LUNA_HEADER2:
        if (c == 0x59) {
            ...
            state = TF_LUNA_HEADER1;
        }
        break;
    case TF_LUNA_DIST_L:
    case TF_LUNA_DIST_H:
    case TF_LUNA_STRENGTH_L:
...
rt_err_t tf_luna_drv_init(const char* uart_dev_name)
{
    dev = rt_device_find(uart_dev_name);

    RT_ASSERT(dev != NULL);

    thread = rt_thread_create("tf_luna", thread_entry, RT_NULL, 4 * 1024, 7, 1);

    RT_ASSERT(thread != NULL);

    RT_CHECK(rt_event_init(&event, "tf_luna", RT_IPC_FLAG_FIFO));
