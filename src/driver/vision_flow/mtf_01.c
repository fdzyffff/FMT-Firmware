...
{
    return rt_event_send(&event, EVENT_MTF01_UPDATE);
}

static bool parse_package(uint8_t c)
{
    static MTF01_State state;
...
static void thread_entry(void* args)
{
    ...
    rt_uint32_t wait_set = EVENT_MTF01_UPDATE;
    uint8_t c;
    ...
    while (1) {
        ...
        if ((res == RT_EOK && (recv_set & EVENT_MTF01_UPDATE)) || res == -RT_ETIMEOUT) {
...
rt_err_t mtf01_drv_init(const char* uart_dev_name)
{
    ...
    thread = rt_thread_create("mtf01", thread_entry, RT_NULL, 8 * 1024, 7, 1);

    RT_ASSERT(thread != NULL);

    RT_CHECK(rt_event_init(&event, "mtf01", RT_IPC_FLAG_FIFO));
